...
struct Statistic{
	WORD distance = 0;	//T�gliche Distanz in m
...
ErrCode drawGraph(Window* window, Font& font, uint x, uint y, uint size_x, uint size_y, DataPoint* data, WORD data_count){
	if(window == nullptr) return WINDOW_NOT_FOUND;
	WORD inc_y = size_y/5;
	drawRectangle(window, x, y, size_x, size_y, RGBA(40, 40, 40));
	drawLine(window, x, y, x+size_x, y, RGBA(70, 70, 70));
	drawLine(window, x, y, x, y+size_y, RGBA(70, 70, 70));
	drawLine(window, x, y+size_y, x+size_x, y+size_y, RGBA(70, 70, 70));
	drawLine(window, x+size_x, y, x+size_x, y+size_y, RGBA(70, 70, 70));
	for(WORD j=1; j < 5; ++j){
		drawLine(window, x, y+inc_y*j, x+size_x, y+inc_y*j, RGBA(70, 70, 70));
	}
	//min, max x
	...
	for(WORD j=0; j < (max_x-min_x)+1; ++j){
		...
		for(size_t k=0; k < txt.size(); ++k){
			drawFontChar(window, font, txt[k], x+5-font.font_size/4+inc_x*j, y+size_y);
		}
	}
	font.font_size = tmp;
	//min, max y
	...
	for(WORD j=0; j < 10; ++j){
		...
		for(size_t k=0; k < txt.size(); ++k){
			offset += drawFontChar(window, font, txt[k], x+offset-total_offset, y+size_y-5-font.font_size/2-inc_y*j);
		}
	}
	font.font_size = tmp;
	...
	for(WORD j=1; j < data_count+1; ++j){
		...
		drawRectangle(window, x+5+pos_x*(size_x-10)-1, y+size_y-5-pos_y*(size_y-10)-1, 3, 3, RGBA(90, 90, 90));
		if(j > 1) drawLine(window, x+5+pos_x*(size_x-10), y+size_y-5-pos_y*(size_y-10), last_x, last_y, RGBA(120, 0, 0));
